...
fn generate_image_parallel(config: &Config, pixels: &ParallelPixelBuffer, w: u32, h: u32, max_iteration: u32) {
  ...
  coords.par_iter().for_each(|&(x, y)| {
    ...
    */
    ...
    let color = map_color(iteration, max_iteration);
...
pub fn main() -> Result<(), String> {
  let config: Config = Config {
    center: [0.,
...
fn benchmark_parrelel(w: u32, h: u32, max_iteration: u32) {
  ...
  let config: Config = Config {center: [0.0, 0.0], density: 4370.246963563};

  generate_image_parallel(&config, &p, w, h, max_iteration);
...
#[test]
fn benchmark() {
  benchmark_parrelel(1000, 1000, 10000);
}
